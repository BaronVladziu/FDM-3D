...
Renderer::Renderer()
{
	...
	window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Project-FDM-3D (PreAlpha)", NULL, NULL);
	if (window == NULL)
	{
	...
	unsigned char *data = stbi_load("Textures/Rectangle.png", &width, &height, &nrChannels, 0);
	if (data)
	{
...
void Renderer::draw(const std::list<Renderable *> & renderables) {
	...
	// render room
	glBindVertexArray(VAO);
	drawVertices(glm::vec3(0.0f, 0.0f, 0.0f), 0, glm::vec3(1.0f, 0.3f, 0.5f), 36);

	// glfw: swap buffers and poll IO events (keys pressed/released, mouse moved etc.)
	glfwSwapBuffers(window);
	glfwPollEvents();
}
void Renderer::drawVertices(const glm::vec3 & translation, float angle, const glm::vec3 & axis, unsigned int numberOfVertices) {
