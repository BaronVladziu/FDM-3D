...
void Renderer::init(const std::list<RenderTriangle> & renderTriangles) {
    ...
    for (const RenderTriangle & renT : renderTriangles) {
        for (const RenderVertex & renV : renT.getVertices()) {
            Vertex v;
            v.pos = glm::vec3(renV.getX(), renV.getY(), renV.getZ());
            v.color = glm::vec3(0,0,0);
            v.texCoord = glm::vec2(renV.getTexX(), renV.getTexY());
            vertices.push_back(v);
            indices.push_back(actVID);
            actVID++;
        }
    }
...
void Renderer::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "FDM-3D", nullptr, nullptr);
...
void Renderer::createInstance() {
    ...
    appInfo.pApplicationName = "Vulkan-FDM-3D";
...
void Renderer::updateUniformBuffer(uint32_t currentImage) {
    ...
    ubo.model = glm::rotate(glm::mat4(1.0f), time * glm::radians(45.0f), glm::vec3(0.0f, 0.0f, 1.0f)); //was "glm::radians(90.0f)" before
